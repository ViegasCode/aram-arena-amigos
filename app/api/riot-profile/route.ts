...
export async function POST(request:Request) {
 try {
  ...
  const cached=await db.prepare('SELECT fetched_at FROM riot_profiles WHERE player_id=? AND lower(riot_id)=lower(?) AND lower(tagline)=lower(?)').bind(p.id,p.riot_id,p.tagline).first<{fetched_at:string}>();
  ...
  const account=await get('americas',`/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(p.riot_id)}/${encodeURIComponent(p.tagline)}`);
  ...
  const saved=await db.batch([
   db.prepare('INSERT INTO riot_profiles(player_id,riot_id,tagline,profile,fetched_at) SELECT id,riot_id,tagline,?,? FROM players WHERE id=? AND riot_id=? AND tagline=? ON CONFLICT(player_id) DO UPDATE SET riot_id=excluded.riot_id,tagline=excluded.tagline,profile=excluded.profile,fetched_at=excluded.fetched_at').bind(JSON.stringify(profile),now,p.id,p.riot_id,p.tagline),
   db.prepare('UPDATE players SET puuid=? WHERE id=? AND riot_id=? AND tagline=?').bind(account.puuid,p.id,p.riot_id,p.tagline),
  ...
  if(!saved[0].meta.changes)return json({error:'O Riot ID mudou durante a consulta. Atualize novamente.'},409);

...
export async function GET() {
 try {
  ...
  const matches=m.results.map((match:any)=>({...match,details:match.details?JSON.parse(match.details):null}));
  return json({ranking:standings(players,r.results),matches,results:r.results,startAt:MAYHEM_START});

...
function withMvp(results:any[],matches:any[],players:any[]){const detailByMatch=new Map(matches.map(m=>[m.id,m.details?.participants||[]]));return results.map(result=>{const contenders=results.filter(other=>other.match_id===result.match_id).map(other=>{const player=players.find(p=>p.id===other.player_id),detail=(detailByMatch.get(result.match_id)||[]).find((d:any)=>d.gameName?.toLowerCase()===player?.riot_id?.toLowerCase()&&d.tagLine?.toLowerCase()===player?.tagline?.toLowerCase());const score=detail?detail.kills*3+detail.assists+detail.damage/1000+detail.healing/2000+detail.shielding/2000-detail.deaths*1.5+(detail.win?5:0):-Infinity;return {id:other.player_id,score};}).sort((a,b)=>b.score-a.score||a.id.localeCompare(b.id));return {...result,mvp:Number.isFinite(contenders[0]?.score)&&contenders[0].id===result.player_id?1:0};});}
export async function GET() {
 try {
  ...
  const matches=m.results.map((match:any)=>({...match,details:match.details?JSON.parse(match.details):null})),results=withMvp(r.results,matches,players);
